...
*--------------------------------------------------------------------*
*    1.00 | 27/03/19  | Francisco Silva     |   Versão Inicial       *
**********************************************************************
REPORT  zrag_carga_zpagfor.
...
TYPES: BEGIN OF ty_input,
         mandt         TYPE zpagfor-mandt,
         lifnr         TYPE zpagfor-lifnr,
         name_lifnr    TYPE zpagfor-name_lifnr,
         saknr_dto     TYPE zpagfor-saknr_dto,
         txt_saknr_dto TYPE zpagfor-txt_saknr_dto,
         saknr_adm     TYPE zpagfor-saknr_adm,
         txt_saknr_adm TYPE zpagfor-txt_saknr_adm,
         dt_ins        TYPE char10,
         usr_ins       TYPE zpagfor-usr_ins,
         dt_updt       TYPE char10,
         usr_updt      TYPE zpagfor-usr_updt,
       END OF ty_input.
...
DATA:      gt_dados   TYPE TABLE OF zpagfor.

*--------------------------------------------------------------------*
* Estruturas                                                         *
*--------------------------------------------------------------------*
DATA:      gs_input   TYPE ty_input.
DATA:      gs_dados   TYPE zpagfor.
...
START-OF-SELECTION.
  ...
  PERFORM f_carrega_arquivo TABLES gt_input USING p_file.
...
FORM f_valida_arquivo.
  ...
  LOOP AT gt_input INTO gs_input.
    ...
    IF gv_tabix IS NOT INITIAL.
      CONCATENATE gs_input-dt_ins+6(4)  gs_input-dt_ins+3(2)  gs_input-dt_ins(2)  INTO gs_dados-dt_ins.
      CONCATENATE gs_input-dt_updt+6(4) gs_input-dt_updt+3(2) gs_input-dt_updt(2) INTO gs_dados-dt_updt.
      IF gs_dados-dt_ins = '00000000'.
        CLEAR gs_dados-dt_ins.
      ENDIF.
      IF gs_dados-dt_updt = '00000000'.
        CLEAR gs_dados-dt_updt.
      ENDIF.
      gs_dados-mandt    = sy-mandt.
      gs_dados-usr_ins  = gs_input-usr_ins.
      gs_dados-usr_updt = gs_input-usr_updt.
...
FORM f_atualiza_tabela.
  ...
  MODIFY zpagfor FROM TABLE gt_dados.
  COMMIT WORK.
